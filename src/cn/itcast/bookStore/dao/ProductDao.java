...
public class ProductDao {
	...
	public List<Object[]> salesList(String year, String month)
			throws SQLException {
		String sql = "SELECT products.name,SUM(orderitem.buynum) totalsalnum FROM orders,products,orderltem WHERE "
				+ "orders.id=orderltem.order_id AND products.id=orderltem.product_id AND orders.paystate=1 and year(ordertime)=? "
				+ "and month(ordertime)=? GROUP BY products.name ORDER BY totalsalnum DESC";
		QueryRunner runner = new QueryRunner(DataSourceUtils.getDataSource());
